...
Vous pouvez utiliser les API serveurs de fichiers pour afficher et gérer vos serveurs de fichiers. Pour plus d'informations sur les serveurs de fichiers, reportez-vous à la section link:sewebiug_create_a_file_server.html["Créez un serveur de fichiers"].
...
`ID`: L'ID du partage de fichiers. `name`: Le nom du partage de fichiers. `Siser_gb`: La taille du partage de fichiers dans Gibs. `niveau_service`: Le nom du niveau de service s'applique: Standard, Premium, Premium-Tiering, Extreme ou Extreme-Tiering. `protocole`: Protocole utilisé pour accéder au partage de fichiers (NFS, CIFS ou multi-protocole). `export_policy': La politique d'exportation du partage de fichiers.
